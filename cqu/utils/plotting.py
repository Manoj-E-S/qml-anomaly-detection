...
class Plotter:
    def __init__(self, selected_features: List | None = None):
    ...
    def plot_roc_auc(self, title, y_test, y_proba):
        ...
        plt.figure(figsize=(8, 6))
        plt.plot(
            fpr, tpr, color="blue", label="ROC Curve (area = {:.2f})".format(roc_auc)
        )
    ...
    def print_results_to_stdout(
        self, title, report, roc_auc, threshold, class_weights=None
    ):
        print(title)
    ...
    def log_model_metrics(self, results):
        ...
        # Format the tables
        classification_report_str = tabulate(
        ...
        output_file = os.path.join(
            output_dir, f"{results['model_name']}_evaluation_results.txt"
        )
